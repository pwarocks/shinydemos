...
exports.renderHomePage = function(tagNames) {
  var homepage = Handlebars.compile(fs.readFileSync(siteconfig.layoutsFolder + '/home.{').toString());
...
exports.renderTagsPages = function(demosByTag) {
  var tagspage = Handlebars.compile(fs.readFileSync(siteconfig.layoutsFolder + '/tag.{').toString()),
...
exports.renderDemos = function() {
  ...
  //we don't want an empty "all" link appearing in the homepage,
  //so we filter it out before rendering.
  exports.renderHomePage(Object.keys(demosByTag).sort().filter(function(tag){
    return tag != "all";
  }));
